...
class Trainer(BaseTrainer):
    ...
    def _evolve(self, Y0) -> torch.Tensor:
        ...
        Args:
            Y0: (B, D) - initial latent state at t=0

        Returns:
            Ypred: (B, T, D) - predicted latent trajectory
        """
        B, D = Y0.shape
        T = self.prediction_length
        m = self.args.context_length  # Context window size

        Ypred = torch.zeros(B, T, D, device=self.device)
        Ypred[:, 0, :] = self.model.knet(Y0.clone())

        for t in range(1, T):
            # Build context window of previous states
            if t < m:
                # Not enough history yet, pad with zeros
                pad_len = m - t
                pad = torch.zeros(B, pad_len, D, device=self.device)
                context = torch.cat([pad, Ypred[:, :t, :]], dim=1)  
            else:
                # Full context window available
                context = Ypred[:, t-m:t, :]  

            # Step 1: Process context with AFT
            aft_output = self.model.aft_layer(context)  
            
            # Step 2: Evolve using Koopman operator
            Ypred[:, t, :] = self.model.knet(aft_output)  
    ...
    def predict_traj(self, X0, steps=50) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for t in range(2, steps+1):
                if t <= m:
                    padding_size = m - len(Y_history) + 1
                    context_list = [torch.zeros_like(Y0.squeeze(0)) for _ in range(padding_size)]
                    context_list.extend(Y_history[1:])
                else:
